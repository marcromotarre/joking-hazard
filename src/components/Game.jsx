...
class Game extends Component {
  ...
  getCardIdByIndex = (hand, index) => (index >= 0 ? hand[index] : -1);
  getHandCardIdByIndex = index => (index >= 0 ? this.state.hand[index] : -1);
  ...
  validatePlayerCard = async () => {
    let gameInfo = collectIdsAndDocs(
      await this.gamesRef.doc(`${this.gameId}`).get()
    );
    const { uid } = auth.currentUser ? auth.currentUser : {};
    let currentPlayer = getPlayerById(gameInfo.players, uid);
